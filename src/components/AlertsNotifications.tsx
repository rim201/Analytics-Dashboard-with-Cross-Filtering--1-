...
function formatResolutionDate(iso: string | undefined, lang: string): string {
  if (!iso) return '';
  try {
...
export default function AlertsNotifications({
  ...
  const resolvedUserName = currentUserName ?? t.alerts.defaultUserName;
  ...
  useEffect(() => {
    ...
    const unsub = subscribeAlerts(
      ...
      (e) => {
        setLoadError(e.message || t.alerts.errorLoad);
        setLoading(false);
      },
    );
    return unsub;
  ...
  const getAlertColor = (type: string) => {
    switch (type) {
      case 'critical':
        return { bg: 'bg-red-50 dark:bg-red-950/70', border: 'border-red-200 dark:border-red-500/40', text: 'text-red-700 dark:text-red-200', iconBg: 'bg-red-100 dark:bg-red-500/15', icon: 'text-red-600 dark:text-red-300' };
      case 'warning':
        return { bg: 'bg-amber-50 dark:bg-amber-950/70', border: 'border-amber-200 dark:border-amber-500/40', text: 'text-amber-700 dark:text-amber-200', iconBg: 'bg-amber-100 dark:bg-amber-500/15', icon: 'text-amber-600 dark:text-amber-300' };
      case 'success':
        return { bg: 'bg-emerald-50 dark:bg-emerald-950/70', border: 'border-emerald-200 dark:border-emerald-500/40', text: 'text-emerald-700 dark:text-emerald-200', iconBg: 'bg-emerald-100 dark:bg-emerald-500/15', icon: 'text-emerald-600 dark:text-emerald-300' };
      default:
        return { bg: 'bg-slate-50 dark:bg-slate-900', border: 'border-slate-200 dark:border-slate-700', text: 'text-slate-700 dark:text-slate-200', iconBg: 'bg-blue-100 dark:bg-slate-800', icon: 'text-blue-600 dark:text-blue-400' };
    }
  };

  const handleMarkResolved = async (id: string) => {
    const name = resolvedUserName.trim() || t.alerts.defaultUserName;
    setActionError(null);
    setActionAlertId(id);
    try {
      ...
      setActionError(t.alerts.errorAction);
    } finally {
  ...
  const handleApprovePending = async (id: string) => {
    ...
    try {
      ...
      setActionError(t.alerts.errorApprove);
    } finally {
      setActionAlertId(null);
    }
  };
  ...
  const handleRejectPending = async (id: string) => {
    ...
    try {
      ...
      setActionError(t.alerts.errorDecline);
    } finally {
      setActionAlertId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t.alerts.title}</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {criticalCount > 0 && (
            <div className="px-3 py-2 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded-xl border border-red-200 dark:border-red-800/50 text-sm font-medium">
              {t.alerts.criticalBadge(criticalCount)}
            </div>
          )}
          {warningCount > 0 && (
            <div className="px-3 py-2 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded-xl border border-amber-200 dark:border-amber-800/50 text-sm font-medium">
              {t.alerts.warningBadge(warningCount)}
            </div>
          )}
          {pendingApprovalCount > 0 && (
            <div className="px-3 py-2 bg-violet-100 dark:bg-violet-900/40 text-violet-800 dark:text-violet-300 rounded-xl border border-violet-200 dark:border-violet-800/50 text-sm font-medium flex items-center gap-1.5">
              ...
              {canModerateAlerts
                ? t.alerts.toValidate(pendingApprovalCount)
                : t.alerts.awaitingValidation(pendingApprovalCount)}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-blue-50 to-emerald-50 dark:from-blue-950/30 dark:to-emerald-950/30 rounded-2xl p-6 border border-blue-200/50 dark:border-blue-800/30">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">{t.alerts.summaryTitle}</h3>
        </div>
        <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
          {pendingApprovalCount > 0
            ? t.alerts.summaryTextWithPending(unresolvedCount, pendingApprovalCount)
            : t.alerts.summaryText(unresolvedCount)}{' '}
          {t.alerts.summaryHint}
        </p>
        <div className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
          <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
          <span>{t.alerts.realtimeSync}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-500 dark:text-gray-400">{t.alerts.statTotal}</span>
        ...
        <div className="bg-red-50 dark:bg-red-950/30 rounded-2xl p-6 shadow-lg border border-red-200 dark:border-red-800/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-red-700 dark:text-red-400">{t.alerts.statCritical}</span>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/30 rounded-2xl p-6 shadow-lg border border-amber-200 dark:border-amber-800/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-amber-700 dark:text-amber-400">{t.alerts.statWarnings}</span>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl p-6 shadow-lg border border-emerald-200 dark:border-emerald-800/60">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-emerald-700 dark:text-emerald-400">{t.alerts.statResolved}</span>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <option value="warning">{t.alerts.filterWarning}</option>
              <option value="info">{t.alerts.filterInfo}</option>
              <option value="success">{t.alerts.filterSuccess}</option>
            </select>
          </div>
          <label className="flex items-center space-x-2 px-4 py-2.5 bg-gray-50 dark:bg-gray-700 rounded-xl cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t.alerts.showResolved}</span>
          </label>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 shadow-lg border border-gray-100 dark:border-gray-700 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{t.alerts.noAlertsFound}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t.alerts.noAlertsHint}</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap flex-col gap-2 text-sm">
                          <div className="flex flex-wrap items-center gap-3">
                            <div className="flex items-center space-x-2">
                              <span className="text-gray-500 dark:text-gray-400">{t.alerts.roomLabel}</span>
                              <span className="font-medium text-gray-900 dark:text-gray-100">{alert.room}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <span className="text-gray-500 dark:text-gray-400">{t.alerts.categoryLabel}</span>
                          ...
                          {alert.status === 'pending' && alert.resolutionRequestedBy && (
                            <div className="mt-1 p-3 bg-white/60 dark:bg-gray-800/60 rounded-xl border border-violet-200/80 dark:border-violet-700/50 text-sm text-gray-800 dark:text-gray-200">
                              <span className="font-medium text-violet-900 dark:text-violet-300">{t.alerts.resolutionRequestLabel}</span>{' '}
                              <span className="font-semibold">{alert.resolutionRequestedBy}</span>
                              {alert.resolutionRequestedAt && (
                                <span className="text-gray-600 dark:text-gray-400">
                                  {' '}· {formatResolutionDate(alert.resolutionRequestedAt, lang)}
                                </span>
                              )}
                            </div>
                          )}
                          ...
                          {alert.status === 'resolved' && (
                            <div className="mt-1 space-y-1 text-sm text-gray-700 dark:text-gray-300">
                              {alert.resolutionRequestedBy && (
                                <div>
                                  <span className="text-gray-500 dark:text-gray-400">{t.alerts.requestedByLabel}</span>{' '}
                                  <span className="font-medium text-gray-900 dark:text-gray-100">{alert.resolutionRequestedBy}</span>
                                  {alert.resolutionRequestedAt && (
                                    <span className="text-gray-600 dark:text-gray-400"> · {formatResolutionDate(alert.resolutionRequestedAt, lang)}</span>
                                  )}
                                </div>
                              )}
                              <div>
                                <span className="text-gray-500 dark:text-gray-400">{t.alerts.resolvedByLabel}</span>{' '}
                                <span className="font-semibold text-emerald-800 dark:text-emerald-400">{alert.resolvedByName ?? '—'}</span>
                                {alert.resolvedAt && (
                                  <span className="text-gray-600 dark:text-gray-400"> · {formatResolutionDate(alert.resolvedAt, lang)}</span>
                                )}
                              </div>
                            </div>
                          )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 ml-auto sm:ml-4">
                      {alert.status === 'open' && (
                        ...
                        >
                          {busy ? '…' : canModerateAlerts ? t.alerts.btnMarkResolved : t.alerts.btnRequestResolution}
                        </button>
                      )}
                      {alert.status === 'pending' && canModerateAlerts && (
                        <div className="flex flex-col sm:flex-row gap-2">
                          ...
                          >
                            {busy ? '…' : t.alerts.btnApproveResolution}
                          </button>
                          <button
                            type="button"
                            disabled={busy}
